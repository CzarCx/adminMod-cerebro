    ...
    <div className="overflow-x-auto">
        ...
        <tbody className="divide-y divide-gray-200">
          {data.map((row) => (
            <tr 
              ...
              className={pageType === 'seguimiento' ? "hover:bg-gray-50 cursor-pointer transition-colors duration-150 group" : ""}
            >
              {pageType === 'seguimiento' && (
                <>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{row.organization}</td>
              ...
              {pageType === 'reportes' && (
                <>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{row.organization}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{row.details}</td>
                </>
              )}
            </tr>
          ))}
      ...
      {pageType === 'seguimiento' && isModalOpen && reportingItem && (
        <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex justify-center items-center z-50 transition-opacity duration-300" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-start space-x-4">
              <div className="flex-shrink-0 h-12 w-12 flex items-center justify-center rounded-full bg-red-100">
                <AlertTriangle />
              </div>
              <div className="flex-1">
                <h2 className="text-xl font-bold mb-1 text-gray-900">Levantar Reporte</h2>
                <p className="text-sm text-gray-500">
                  ID del Registro: <span className="font-semibold text-gray-700">{reportingItem.id}</span>
                </p>
              </div>
            </div>

            <div className="mt-6 bg-gray-50 p-4 rounded-lg border border-gray-200 text-sm">
                Estás reportando el producto <strong className="text-gray-800">{reportingItem.product}</strong>
                {' '} de la empresa <strong className="text-gray-800">{reportingItem.organization}</strong>,
                {' '} asignado a <strong className="text-gray-800">{reportingItem.name}</strong>.
            </div>
            ...
            <div className="mt-8 flex justify-end space-x-3">
              <button onClick={() => setIsModalOpen(false)} className="px-5 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 font-semibold text-sm transition-colors">Cancelar</button>
              <button onClick={handleSaveReport} className="px-5 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-bold text-sm shadow-sm hover:shadow-md transition-all duration-200 flex items-center space-x-2">
